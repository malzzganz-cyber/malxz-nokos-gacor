...
router.post("/testimonials", optionalAuth, async (req, res) => {
  ...
  const name = typeof body["name"] === "string" ? body["name"].trim() : req.user?.name?.trim() || "";
...
// Real stats from Firestore counts; falls back to a reasonable estimate if unavailable.
